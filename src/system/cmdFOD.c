...
int help(char *fmt, char *params, int nparams) {
    char data[];
    sprintf(&data, "%d", GET_VERSION);
    i2c_write(&data);
    char ver[];
    i2c_read(&ver);
    sprintf(&data, "%d", FOD_GET_STATUS);
    i2c_write(data);
    char status;
    i2c_read(&status);
...
int deploy_femtosats(char *fmt, char *params, int nparams) {
    ...
    if (i2c_write(DEPLOY_FEMTOSATS, params)) {
        i2c_write(FOD_GET_STATUS, NULL); 
	int res = i2c_read();
	if (res)
	    LOGI(tag, "Released");
	else
...
int get_status(char *fmt, char *params, int nparams) {
    ...
    if (i2c_write(FOD_GET_STATUS, params))
	return CMD_OK;
    return CMD_FAIL;
}
...
int get_femtosat_data(char *fmt, char *params, char nparams) {
    ...
    if (i2c_write(GET_FEMTOSAT_DATA, params))
	return CMD_OK;
    return CMD_FAIL;
}
...
int send_beacon(char *fmt, char *params, char nparams) {
    ...
    if (i2c_write(SEND_BEACON, params))
	return CMD_OK;
    return CMD_FAIL;
}
...
int enable_low_power_mode(char *fmt, char *params, char nparams) {
    ...
    if (i2c_write(EMABLE_LOW_POWER_MODE, params))
	return CMD_OK;
    return CMD_FAIL;
}
...
int disable_low_power_mode(char *fmt, char *params, char nparams) {
    ...
    if (i2c_write(DISABLE_LOW_POWER_MODE, params))
	return CMD_OK;
    return CMD_FAIL;
}
...
int get_version(char *fmt, char *params, char nparams) {
    ...
    if (i2c_write(GET_VERSION, params)) {
	char ver[];
	i2c_read(ver);
...
int i2c_write(char data[]) {
    ...
    if (init_ok) {
        ...
        uint8_t result = bcm2835_i2c_write(data, 3);
